...
public class TreasureChest : MonoBehaviour, IInteractable, ISaveable
{
    ...
    public void SpawnItems()
    {
        ...
        for (int i = 0; i < dropCount; i++)
        {
            ...
            float rx = UnityEngine.Random.Range(1, spreadX) * dir;

            var force = new Vector2(rx, launchForceY);
            var item = obj.GetComponent<WorldItem>();
            if (item) item.SetupAsDroppedItem(force);
        }
    }
    ...
    {
        if (state is not ChestSaveData s || !s.isOpen) return;
        _isOpen = true;
        _isOpening = false;
